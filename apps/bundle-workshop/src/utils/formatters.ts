...
export const formatCurrency = (v: number, code = "USD") =>
...
export const formatOrdinal = (v: number) =>
...
export const formatMonth = (d: Date | string | number) =>
  new Date(d).toLocaleDateString("en-US", { year: "numeric", month: "long" });
export const formatQuarter = (d: Date | string | number) => {
  const dt = new Date(d);
  return `Q${Math.floor(dt.getMonth() / 3) + 1} ${dt.getFullYear()}`;
};
export const formatWeek = (d: Date | string | number) => {
  const dt = new Date(d);
  const jan1 = new Date(dt.getFullYear(), 0, 1);
  return `Week ${Math.ceil(((dt.getTime() - jan1.getTime()) / 86400000 + jan1.getDay() + 1) / 7)}`;
};

export const formatPhoneUs = (v: string) =>
  v.replace(/\D/g, "").replace(/^(\d{3})(\d{3})(\d{4})$/, "($1) $2-$3");
export const formatZip = (v: string) => v.replace(/\D/g, "").slice(0, 5);
export const formatCreditCard = (v: string) =>
  v
    .replace(/\D/g, "")
    .replace(/(.{4})/g, "$1 ")
    .trim();
export const formatSsn = (v: string) =>
  v.replace(/\D/g, "").replace(/^(\d{3})(\d{2})(\d{4})$/, "$1-$2-$3");
export const formatEin = (v: string) => v.replace(/\D/g, "").replace(/^(\d{2})(\d{7})$/, "$1-$2");

export const truncate = (s: string, max: number, suffix = "…") =>
  s.length > max ? s.slice(0, max - suffix.length) + suffix : s;
...
export const highlight = (text: string, query: string) =>
  text.replace(new RegExp(`(${query.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})`, "gi"), "**$1**");
