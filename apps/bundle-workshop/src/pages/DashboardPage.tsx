...
const METRICS = [
  { label: "Monthly revenue",   value: 128400,  prev: 115200,  format: (v: number) => fmt.formatCurrency(v) },
  { label: "Active users",      value: 48291,   prev: 44100,   format: (v: number) => fmt.formatNumber(v)   },
  { label: "Conversion rate",   value: 0.0342,  prev: 0.0318,  format: (v: number) => fmt.formatPercent(v)  },
  { label: "Avg order value",   value: 84.60,   prev: 79.20,   format: (v: number) => fmt.formatCurrency(v) },
...
const RECENT = [
  ...
  { id:"INV-0840", customer:"Globex Industries", amount: 1850,  date:"2026-05-12", status:"pending" },
  ...
  { id:"INV-0837", customer:"Soylent Green Inc", amount: 2100,  date:"2026-05-10", status:"paid"    },
];
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 32 }}>
        {METRICS.map(({ label, value, prev, format }) => {
          ...
          return (
            <div key={label} style={{ padding: "14px 16px", border: "1px solid #e5e7eb", borderRadius: 8, background: "#fff" }}>
        ...
        <tbody>
          {RECENT.map(row => (
            <tr key={row.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
              ...
              <td style={{ padding: "10px 12px", fontWeight: 600 }}>{fmt.formatCurrency(row.amount)}</td>
              <td style={{ padding: "10px 12px", color: "#6b7280" }}>{fmt.formatDate(row.date)}</td>
              <td style={{ padding: "10px 12px" }}>
                <span style={{ padding: "2px 8px", borderRadius: 12, fontSize: 12, background: STATUS_COLOR[row.status] + "22", color: STATUS_COLOR[row.status] }}>
                  {row.status}
                </span>
              </td>
            </tr>
