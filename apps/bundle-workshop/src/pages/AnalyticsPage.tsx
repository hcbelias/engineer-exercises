...
import { PALETTE_DEFAULT, AXIS_FORMATS, ticks, rollingAverage, createConfig } from "../lib/chartEngine";
import { formatCurrency, formatCompact } from "../utils/formatters";

// ─── Fake time-series data ─────────────────────────────────────────────────

function seed(s: number) { let n = s; return () => { n = Math.imul(1664525, n) + 1013904223; return (n >>> 0) / 0xffffffff; }; }
...
const config = createConfig("line", MONTHS, [
  { label:"Revenue",        data: REVENUE,       backgroundColor: PALETTE_DEFAULT[0], borderColor: PALETTE_DEFAULT[0] },
  { label:"3-mo avg",       data: smoothRevenue,  backgroundColor: PALETTE_DEFAULT[1], borderColor: PALETTE_DEFAULT[1] },
  { label:"Operating cost", data: COSTS,          backgroundColor: PALETTE_DEFAULT[2], borderColor: PALETTE_DEFAULT[2] },
]);
...
function MiniBar({ values, color }: { values: number[]; color: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 80 }}>
      {values.map((v, i) => (
        <div key={i} title={`${MONTHS[i]}: ${formatY(v)}`} style={{ flex: 1, background: color, opacity: 0.8, height: `${(v / max) * 100}%`, borderRadius: "2px 2px 0 0", cursor: "default", transition: "opacity 0.2s" }} />
      ))}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 13, color: "#6b7280", margin: "0 0 24px" }}>
        Chart series: {config.datasets.map(d => d.label).join(", ")} ·{" "}
        Y-axis: {yTicks.map(t => formatY(t)).join(", ")}
      </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 28 }}>
        {[
          ...
          <div key={label} style={{ padding: "14px 16px", border: "1px solid #e5e7eb", borderRadius: 8 }}>
