...
export const PALETTE_DEFAULT = ["#6366f1","#8b5cf6","#ec4899","#f43f5e","#f97316","#eab308","#22c55e","#14b8a6","#0ea5e9","#3b82f6"];
export const PALETTE_PASTEL  = ["#c7d2fe","#ddd6fe","#fbcfe8","#fecdd3","#fed7aa","#fef08a","#bbf7d0","#99f6e4","#bae6fd","#bfdbfe"];
export const PALETTE_DARK    = ["#312e81","#4c1d95","#831843","#881337","#7c2d12","#713f12","#14532d","#134e4a","#0c4a6e","#1e3a8a"];
export const PALETTE_MONO    = ["#f9fafb","#f3f4f6","#e5e7eb","#d1d5db","#9ca3af","#6b7280","#4b5563","#374151","#1f2937","#111827"];
export const PALETTE_DIVERGE = ["#dc2626","#ea580c","#d97706","#ca8a04","#65a30d","#16a34a","#059669","#0891b2","#2563eb","#7c3aed"];
...
export function sqrtScale(domain:[number,number],range:[number,number]) {
  return (v:number)=>linearScale([Math.sqrt(domain[0]),Math.sqrt(domain[1])],range)(Math.sqrt(v));
}
...
export function normalise(v:number,min:number,max:number){return clamp((v-min)/(max-min),0,1);}
...
export const AXIS_FORMATS = {
  ...
  currency:   (v:number)=>Intl.NumberFormat("en",{style:"currency",currency:"USD"}).format(v),
  date:       (v:number)=>new Date(v).toLocaleDateString(),
  datetime:   (v:number)=>new Date(v).toLocaleString(),
  duration:   (v:number)=>{const h=Math.floor(v/3600),m=Math.floor(v%3600/60),s=v%60;return`${h?h+"h ":""}${m?m+"m ":""}${s}s`;},
  bytes:      (v:number)=>{const u=["B","KB","MB","GB","TB"];let i=0;while(v>=1024&&i<u.length-1){v/=1024;i++;}return`${v.toFixed(1)} ${u[i]}`;},
  scientific: (v:number)=>v.toExponential(2),
  ordinal:    (v:number)=>`${v}${["th","st","nd","rd"][v%10>3||~~(v%100-v%10)===10?0:v%10]}`,
};
...
export function stackSeries(data:Record<string,number>[]){
  ...
  return data.map(row=>{let acc=0;return Object.fromEntries(keys.map(k=>{const v=row[k]??0;const entry=[k,[acc,acc+v]];acc+=v;return entry;}));});
}
export function rollingAverage(values:number[],window:number){
  return values.map((_,i)=>{const s=Math.max(0,i-window+1);return values.slice(s,i+1).reduce((a,b)=>a+b,0)/(i-s+1);});
}
export function groupBy<T>(arr:T[],key:keyof T):Map<T[keyof T],T[]>{
  return arr.reduce((m,item)=>{const k=item[key];if(!m.has(k))m.set(k,[]);m.get(k)!.push(item);return m;},new Map<T[keyof T],T[]>());
}
export function extent(values:number[]):[number,number]{return[Math.min(...values),Math.max(...values)];}
export function bin(values:number[],thresholds:number[]){
  const bins:number[][]=Array.from({length:thresholds.length+1},()=>[]);
  values.forEach(v=>{const i=thresholds.findIndex(t=>v<t);bins[i===-1?bins.length-1:i].push(v);});
  return bins;
}
export function kde(kernel:(v:number)=>number,thresholds:number[],data:number[]){
  return thresholds.map(x=>({x,y:data.reduce((s,v)=>s+kernel(x-v),0)/data.length}));
}
export const epanechnikovKernel=(bandwidth:number)=>(v:number)=>Math.abs(v/=bandwidth)<=1?0.75*(1-v*v)/bandwidth:0;
...
export type ChartType = "bar"|"line"|"pie"|"doughnut"|"radar"|"scatter"|"bubble"|"area"|"histogram"|"heatmap";
export type LegendPosition = "top"|"bottom"|"left"|"right";
export interface ChartDataset { label:string; data:number[]; backgroundColor?:string|string[]; borderColor?:string; }
export interface ChartConfig { type:ChartType; labels:string[]; datasets:ChartDataset[]; }

export function createConfig(type:ChartType,labels:string[],datasets:ChartDataset[]):ChartConfig{
