...
export function contentBox(page: { width: number; height: number }, m: Margin): Box {
  return { x: m.left, y: m.top, width: page.width - m.left - m.right, height: page.height - m.top - m.bottom };
}
...
export interface TableColumn { header: string; key: string; width?: number; align?: "left"|"center"|"right"; format?: (v: unknown) => string; }
...
export interface DocumentOptions { title: string; author: string; subject?: string; pageSize?: PageSize; orientation?: Orientation; margins?: Partial<Margin>; }
...
export function createDocument(options: DocumentOptions) {
  ...
  return { ...options, page, margin: m, content: contentBox(page, m), createdAt: new Date().toISOString() };
}
