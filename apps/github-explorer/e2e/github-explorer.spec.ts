...
test.describe("GitHub Explorer", () => {
  test("searching a valid username shows user card and repos", async ({ page }) => {
    ...
    const searchInput = page.locator('input[placeholder*="username" i], input[placeholder*="search" i], input[type="search"]').first();
  ...
  test("Load more button appends repos without replacing previous ones", async ({ page }) => {
    ...
    const repoItems = page.locator('[class*="repo-card"], [data-testid="repo-card"], [class*="RepoCard"]');
  ...
  test("Star button updates immediately (optimistic UI)", async ({ page }) => {
    ...
    const starButton = page.locator('button').filter({ hasText: /star|unstar/i }).first();
